...
- Storing Messages within each User Document
	...
	- Pros:
		- It is easy to see who a message belongs to (i.e. who the recipient is) since the message is already in that user's document (kind of like an inbox).
	- Cons:
...
For iOS App, we are using 'Xcode' as the platform to work on for UI. We use 'Figma' to design different screens of the app and code them into Xcode.
